...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && index >=0 && index < ll_len(this))
    {
        if(index == 0)
        {
            pNodoToDelete = this->pFirstNode;
            this->pFirstNode = getNode(this, 1);

        }else{
            pNodoAdelante = getNode(this, index - 1);

            pNodoToDelete = pNodoAdelante->pNextNode;

            pNodoAdelante->pNextNode = pNodoToDelete->pNextNode;
        }
        free(pNodoToDelete);
        (this->size)--;
        returnAux=0;
...
/** \brief Devuelve un nuevo LinkeList cargado con los elementos que pasaron el filtro
 *
...
{
    int tam;
    LinkedList* nuevaLinkedList = ll_newLinkedList();
    eUsuario* aux = NULL;
